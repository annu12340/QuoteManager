...
def test_for_Empty_Symbol():
    with pytest.raises(KeyError):
     ...
     assert QuoteManager1.execute_Trade_dic('', 100) == None
...
def test_for_NonExistent_Symbol():
    with pytest.raises(KeyError):
     ...
     assert QuoteManager1.execute_Trade_dic('Non existent symbol', 100) == None
...
def test_for_Existing_Symbol():
    with pytest.raises(KeyError):
     ...
     assert QuoteManager1.execute_Trade_dic('A', 1) != None
...
def test_for_special_cases():
    ...
    assert QuoteManager1.execute_Trade_dic("smallest_symbol",900) == None
